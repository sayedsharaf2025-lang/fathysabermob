function parseExcelCustomers(data) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (headerFound || i === 0) {
      ...
      if (name && phone) {
        customers.push({ name, phone, planPrice: parseFloat(row[2]) || 0, plan: String(row[3] || 'مفوتر').trim(), nationalId: String(row[4] || '').trim() });
      }
    }
  }
  return customers;
...
function parseExcelInvoices(data) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (headerFound || i === 0) {
      ...
      if (phone && amount > 0) {
        invoices.push({ phone, plan: String(row[1] || 'مفوتر').trim(), amount });
      }
    }
  }
  return invoices;
